...
{
int hashedString = hash_string(item,this->size);

listset_add(&this->table[hashedString],item);
...
{
int hashed_item = hash_string(item,this->size);
  return listset_lookup(&this->table[hashed_item],item);
}
...
void hashtable_remove(struct hashtable * this, char * item)
{
  ...
  listset_remove(&this->table[hashed_item],item);
}
